...
pub struct TransactionVar {
    ...
    pub recipient_signature: Option<SignatureVar<EdwardsProjective, EdwardsVar>>
}
...
impl TransactionVar {
    ...
    pub fn validate(
        ...
        let sender_exists = pre_sender_path
            .verify_membership(
        ...
        let sender_updated_correctly = post_sender_path
            .verify_membership(
                &parameters.leaf_crh_params,
                &parameters.two_to_one_crh_params,
                post_root,
        ...
        let recipient_updated_correctly = post_recipient_path
            .verify_membership(
                &parameters.leaf_crh_params,
                &parameters.two_to_one_crh_params,
                post_root,
...
impl AllocVar<Transaction, ConstraintF> for TransactionVar {
    ...
    fn new_variable<T: Borrow<Transaction>>(
        ...
        f().and_then(|tx| {
            ...
            let recipient_signature = match tx.recipient_signature.clone() {
                Some(r) => Some(SignatureVar::new_variable(cs, || Ok(&r), mode)?),
...
mod test {
    ...
    fn unary_rollup_validity_test() {
        ...
        state
            .update_balance(alice_id, Amount(2000))
        ...
        let tx1 = Transaction::create(&pp, alice_id, bob_id, Amount(1001), &alice_sk, Some(&bob_sk), &mut rng);
        println!("tx1: {:?}, bob_sk: {:?}", tx1.recipient_signature, tx1.amount);
